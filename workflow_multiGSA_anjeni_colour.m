...
[hydro_mvd_rsa,hydro_idxb_rsa] = RSA_indices_thres(X,[hydro_NSEmod hydro_PBIASmod hydro_KGEmod],hydro_threshold);
...
figure; scatter_plots(X,hydro_nse,[],'NSE',X_Labels,hydro_idxb_rsa);
for i=1:M
...
% Plot parameter CDFs:
RSA_plot_thres(X,hydro_idxb_rsa,[],X_Labels,{'behav','non-behav'}); % add legend
...
Nboot = 100;
[hydro_mvd,hydro_idxb,hydro_mvd_lb,hydro_mvd_ub] = RSA_indices_thres(X,[hydro_NSEmod hydro_PBIASmod hydro_KGEmod],hydro_threshold,1,Nboot);
[hydro_spread,~,hydro_spread_lb,hydro_spread_ub] = RSA_indices_thres(X,[hydro_NSEmod hydro_PBIASmod hydro_KGEmod],hydro_threshold,2,Nboot);
% Plot results:
...
for i = 1:Nbeh
    XY(N2*(i-1)+1:N2*i,1:M) = repmat(hydro_Xbeh(i,:),N2,1);
end
Y = XY(:,M+1:M+E);
...
% Correspondent hydrological evaluation for erosion parameters
erosion_hydro_kge = KGE(Qero(:,warmup+1:end),flow(warmup+1:end)');
erosion_hydro_KGEmod = 1 - (erosion_hydro_kge); % 1-KGE to minimise
erosion_hydro_kge_sortindx = [];
[erosion_hydro_kge_sortindx(:,1), erosion_hydro_kge_sortindx(:,2)] = sort(erosion_hydro_KGEmod);
erosion_hydro_KGEnorm = (erosion_hydro_KGEmod-min(hydro_KGEmod))/(max(hydro_KGEmod)-min(hydro_KGEmod));
...
% mvd: maximum vertical distance between parameters CDFs
% Plot results:
figure; boxplot1([hydro_mvd,erosion_mvd],XY_Labels,'MVD',[hydro_mvd_lb,erosion_mvd_lb],[hydro_mvd_ub,erosion_mvd_ub])
grid on
...
disp('Done'); disp(' ')
%% Plot simulated time series:
disp('Plot simulated time series:')
...
e = plot(manual_Qsim(4,:)','LineWidth',1,'LineStyle',':','Color',[1 0.25 0.25]);
% Customise the plot
title('Flow')
...
e = plot(manual_Csim(4,:)','LineWidth',1,'LineStyle',':','Color',[1 0.25 0.25]);
% Fraction H
...
set(gca,'XTick',(0:200:T+1),'XTickLabels',datestr((0:200:T+1)+datenum('01-Jan-1990'),'dd/mm/yyyy'))
...
subplot(4,3,4:5)
parcoor(X,X_Labels,[],hydro_idxb_rsa,12)
hold on
...
hold on
b = plot(erosion_nse(erosion_idxb_rsa),erosion_pbias(erosion_idxb_rsa),'ok','MarkerSize',3,'MarkerFaceColor','k');
% Threshold
...
disp('Done'); disp(' ')
%% 23. RSA USING NSE, PBIAS, AND KGE AS OBJECTIVE FUNCTIONS
disp('RSA USING NSE, PBIAS, AND KGE AS OBJECTIVE FUNCTIONS')
...
disp('Done'); disp(' ')
%% 24. EVALUATION OF THE RANDOM HYDROLOGICAL MODULE
...
figure
subplot(2,2,1)
boxplot1([hydro_mvd,erosion_mvd],XY_Labels,'mvd',[hydro_mvd_lb,erosion_mvd_lb],[hydro_mvd_ub,erosion_mvd_ub],12)
grid on
title('(a) Sensitivity using behavioural hydrological sets')

subplot(2,2,3)
boxplot1([hydro_spread,erosion_spread],XY_Labels,'spread',[hydro_spread_lb,erosion_spread_lb],[hydro_spread_ub,erosion_spread_ub],12)
grid on
set(gca,'YLimMode','auto')
...
% a = plot(hydro_nse,hydro_pbias,'o','MarkerSize',3,'Color',[.5 .5 .5]);
x = hydro_KGEnorm;
for j = 1:length(hydro_KGEnorm)
    a = plot(hydro_nse(j),hydro_pbias(j),'ok','MarkerSize',3,'MarkerFaceColor','none',...
        'MarkerEdgeColor',0.75-[(1-x(j))*0.25 x(j)*0.25 abs(0.25-x(j))*0.75]);
end
% Threshold
c = plot([hydro_nse_thres hydro_nse_thres],hydro_pbias_thres*[-1 1],'--k');
plot([hydro_nse_thres 1],[hydro_pbias_thres hydro_pbias_thres],'--k');
plot([hydro_nse_thres 1],-1*[hydro_pbias_thres hydro_pbias_thres],'--k');
% Behavioural sets
plot(hydro_nse(hydro_idxb_rsa),hydro_pbias(hydro_idxb_rsa),'ok','MarkerSize',3,...
        'MarkerFaceColor','k',...
        'MarkerEdgeColor','none');
% auxidxb = find(hydro_idxb_rsa);
% for j = 1:length(auxidxb)
%     plot(hydro_nse(auxidxb(j)),hydro_pbias(auxidxb(j)),'ok','MarkerSize',2,...
%         'MarkerFaceColor',(1-hydro_KGEnorm(auxidxb(j)))*[1 0.25 0.25],...
%         'MarkerEdgeColor','none');
% end
% Plot best predictions:
d = plot(hydro_nse_bestObj(1),hydro_nse_bestObj(2),'vk','MarkerSize',10,'MarkerFaceColor',0.8*[1 1 1]);
...
text(0.5,30,'Performance threshold (Moriasi et al., 2007)')
% text(0.41,5,'  Moriasi et al. (2007) threshold')
% Customise plot
set(gca,'XLim',[0.3 0.9],'YLim',[-60 60],'XTick',(0:0.05:1),'YTick',(-100:20:100))
set(gca,'FontName','Helvetica','FontSize',12)
xlabel('NSE [-]'); ylabel('PBIAS [%]')
title('(a) Hydrological module')
box on
...
% grid minor
% Legend
b1 = plot(3,0,'ok','MarkerSize',3,'MarkerFaceColor','none',...
    'MarkerEdgeColor',0.75-[(1-1)*0.25 1*0.25 abs(0.25-1)*0.75]);
b2 = plot(2,0,'ok','MarkerSize',3,'MarkerFaceColor','none',...
    'MarkerEdgeColor',0.75-[(1-0)*0.25 0*0.25 abs(0.25-0)*0.75]);
legend([b1 b2],'Bad','Good');
legend('location','southeast')
% legend('orientation','horizontal')
legend('boxoff')
% legend('off')

clear a b c d e f g h i auxidxb j legx lpox b1 b2
...
x = rand_erosion_KGEnorm;
for j = 1:length(rand_erosion_KGEnorm)
    a = plot(rand_hydro_full_nse(j),rand_hydro_full_pbias(j),'ok','MarkerSize',3,'MarkerFaceColor','none',...
        'MarkerEdgeColor',0.75-[(1-x(j))*0.25 x(j)*0.25 abs(0.25-x(j))*0.75]);
end
% Threshold
...
% b = plot(rand_hydro_full_nse(rand_hydro_full_multi_idxb_rsa),rand_hydro_full_pbias(rand_hydro_full_multi_idxb_rsa),'ok','MarkerSize',3,'MarkerFaceColor','k');
% Plotting behavioural erosion into hydrolgical module
% i = plot(rand_hydro_nse),rand_hydro_pbias,'sk','MarkerSize',3,'MarkerFaceColor','k');
i2 = plot(rand_hydro_nse(~rand_hydro_M2007),rand_hydro_pbias(~rand_hydro_M2007),'sr','MarkerSize',3,'MarkerFaceColor','r');
i1 = plot(rand_hydro_nse(rand_hydro_M2007),rand_hydro_pbias(rand_hydro_M2007),'sk','MarkerSize',3,'MarkerFaceColor','k');
% Plot best predictions:
...
% grid minor
% Legend
legend([i1 i2],'Acceptable','Unnaceptable')
legend('location','southeast')
legend('boxoff')
% legend('off')

clear a b c d e f g h i i1 i2 j x
...
% Plot all simulations:
% a = plot(erosion_nse,erosion_pbias,'o','MarkerSize',3,'Color',[.5 .5 .5]);
x = erosion_hydro_KGEnorm;
for j = 1:length(erosion_hydro_KGEnorm)
    a = plot(erosion_nse(j),erosion_pbias(j),'ok','MarkerSize',3,'MarkerFaceColor','none',...
        'MarkerEdgeColor',0.75-[(1-x(j))*0.25 x(j)*0.25 abs(0.25-x(j))*0.75]);
end
% Threshold
c = plot([erosion_nse_thres erosion_nse_thres],erosion_pbias_thres*[-1 1],'--k');
...
% grid minor
% Legend
b = plot(2,0,'ok','MarkerFaceColor','k','MarkerSize',3);
i = plot(2,0,'sk','MarkerFaceColor','k','MarkerSize',3);
% legend([a c b i d e f g h],'Parameter population','Threshold','Behavioural erosion sets','Best NSE set','Best PBIAS set','Best KGE set','Best set overall','Manual optima')
legend([b i],'Behavioural hydrological sets','Behavioural erosion sets');
legend('location','southeast')
legend('boxoff')
% legend('off')

clear a b c d e f g h i j x

% subplot(2,2,4)
subplot(7,7,[33:35 40:42 47:49])
hold on
% Plot all simulations:
% a = plot(rand_erosion_nse,rand_erosion_nse,'o','MarkerSize',3,'Color',[.5 .5 .5]);
x = rand_hydro_full_KGEnorm;
for j = 1:length(rand_hydro_full_KGEnorm)
    a = plot(rand_erosion_nse(j),rand_erosion_pbias(j),'ok','MarkerSize',3,'MarkerFaceColor','none',...
        'MarkerEdgeColor',0.75-[(1-x(j))*0.25 x(j)*0.25 abs(0.25-x(j))*0.75]);
end
% Threshold
...
% i = plot(rand_erosion_nse(rand_erosion_idxb_rsa),rand_erosion_pbias(rand_erosion_idxb_rsa),'sk','MarkerSize',3,'MarkerFaceColor','k');
i2 = plot(rand_erosion_nse(and(~rand_hydro_full_M2007,rand_erosion_idxb_rsa)),rand_erosion_pbias(and(~rand_hydro_full_M2007,rand_erosion_idxb_rsa)),'sr','MarkerSize',3,'MarkerFaceColor','r');
i1 = plot(rand_erosion_nse(and(rand_hydro_full_M2007,rand_erosion_idxb_rsa)),rand_erosion_pbias(and(rand_hydro_full_M2007,rand_erosion_idxb_rsa)),'sk','MarkerSize',3,'MarkerFaceColor','k');
% Plot best prediction:
...
legend([d e f g],'Best NSE set','Best PBIAS set','Best KGE set','Best set overall')
legend('boxoff')
% legend('off')
...
disp('Exporting the Figure to pdf')
% Set the paper size [width height]
set(fig_2,'PaperSize',[40 20]);
% set(fig_2,'PaperOrientation','landscape')
print(fig_2,'Fig2_export','-dpdf','-fillpage')
...
% Parallel coordinate plot:
subplot(6,3,4:5)
parcoor(X,X_Labels,[],hydro_idxb_rsa,10)
hold on
multi_bestnorm_nse = (hydro_nse_sortset(1,:)-min(X))./(max(X)-min(X));
multi_bestnorm_pbias = (hydro_pbias_sortset(1,:)-min(X))./(max(X)-min(X));
multi_bestnorm_kge = (hydro_kge_sortset(1,:)-min(X))./(max(X)-min(X));
...
[ hydro_pawn_Tm, hydro_pawn_lb, hydro_pawn_ub ] = pawn_indices(hydro_pawn_Yu,hydro_pawn_YY,stat,[],Nboot);
...
% Calculate RSA comparable to PAWN
[hydro_multi_mvd,~,hydro_multi_mvd_lb,hydro_multi_mvd_ub] = RSA_indices_thres(X,hydro_multi,hydro_multi_thres,1,Nboot);

% Plot:
figure; boxplot1(hydro_pawn_Tm,X_Labels,'pawn',hydro_pawn_lb,hydro_pawn_ub)
...
stat = 'max' ; % statistic to be applied to KSs
Nboot = 100  ; % number of boostrap resamples
[ rand_erosion_pawn_Tm, rand_erosion_pawn_lb, rand_erosion_pawn_ub ] = pawn_indices(rand_erosion_pawn_Yu,rand_erosion_pawn_YY,stat,[],Nboot);
...
% Calculate RSA comparable to PAWN
[rand_erosion_multi_mvd,~,rand_erosion_multi_mvd_lb,rand_erosion_multi_mvd_ub] = RSA_indices_thres(XY2,rand_erosion_multi,rand_erosion_multi_thres,1,Nboot);

% Plot:
figure; boxplot1(rand_erosion_pawn_Tm,XY_Labels,'pawn',rand_erosion_pawn_lb,rand_erosion_pawn_ub)
title('Model performance')

disp('')
%% 37. APPLY PAWN FOR THE RANDOM EROSION MODULE AND FOR PEAK CONCENTRATIONS
disp('APPLY PAWN FOR THE RANDOM EROSION MODULE AND FOR PEAK CONCENTRATIONS')
...
for j = 1:n
    for h = 1:E
        ...
        for i = 1:Nbeh
            erosion_pawn_XY{h,j}(NC*(i-1)+1:NC*i,1:M) = repmat(hydro_Xbeh(i,:),NC,1);
        end
...
stat = 'max' ; % statistic to be applied to KSs
Nboot = 100  ; % number of boostrap resamples
[ erosion_pawn_Tm, erosion_pawn_lb, erosion_pawn_ub ] = pawn_indices(erosion_pawn_Yu,erosion_pawn_YY,stat,[],Nboot);
...
% Calculate RSA comparable to PAWN
[erosion_multi_mvd,~,erosion_multi_mvd_lb,erosion_multi_mvd_ub] = RSA_indices_thres(Y,erosion_multi,erosion_multi_thres,1,Nboot);

% Plot:
figure; boxplot1(erosion_pawn_Tm,Y_Labels,'pawn',erosion_pawn_lb,erosion_pawn_ub)
title('Model performance')

disp('')
%% 41. APPLY PAWN FOR THE BEHAVIOURAL EROSION MODULE AND FOR PEAK CONCENTRATIONS
disp('APPLY PAWN FOR THE BEHAVIOURAL EROSION MODULE AND FOR PEAK CONCENTRATIONS')
...
figure
subplot(2,2,1)
boxplot1([hydro_mvd,erosion_mvd],XY_Labels,'mvd',[hydro_mvd_lb,erosion_mvd_lb],[hydro_mvd_ub,erosion_mvd_ub],10)
hold on
...
% figure
subplot(2,2,2)
boxplot1([hydro_pawn_Tm,erosion_pawn_Tm],XY_Labels,'pawn',[hydro_pawn_lb,erosion_pawn_lb],[hydro_pawn_ub,erosion_pawn_ub],10)
hold on
plot([M M]+0.5,[-0.5 1.5],'--k')
grid on
...
subplot(2,2,4)
boxplot1(rand_erosion_pawn_Tm,XY_Labels,'pawn',rand_erosion_pawn_lb,rand_erosion_pawn_ub,10)
grid on
...
disp('')
%% 45. APPLY PAWN GENERIC SAMPLING FOR THE HYDROLOGICAL MODULE
disp('APPLY PAWN GENERIC SAMPLING FOR THE HYDROLOGICAL MODULE')

n  = 10 ; % number of conditioning points

% Generic sampling + splitting (Pianosi and Wagener, 2018):
[hydro_genpawn_KS,hydro_genpawn_KS_Tmdummy,hydro_genpawn_YF,hydro_genpawn_Fu,hydro_genpawn_Fc,hydro_genpawn_YY2,hydro_genpawn_xc2,hydro_genpawn_NC,hydro_genpawn_XX2,hydro_genpawn_YU,hydro_genpawn_idx_bootstrap] = pawn_ks_givendata(X,hydro_multi,n);
hydro_genpawn_max_Tm = nanmax(hydro_genpawn_KS);
hydro_genpawn_median_Tm = nanmedian(hydro_genpawn_KS);
hydro_genpawn_mean_Tm = nanmean(hydro_genpawn_KS);
hydro_genpawn_min_Tm = nanmin(hydro_genpawn_KS);
% Plot:
figure;
subplot(2,2,1); boxplot1(hydro_genpawn_max_Tm,X_Labels,'pawn') 
hold on; plot((1:M),ones(1,M)*hydro_genpawn_KS_Tmdummy,'r','LineWidth',2)
title('Model performance (max stat)')
subplot(2,2,2); boxplot1(hydro_genpawn_median_Tm,X_Labels,'pawn') 
hold on; plot((1:M),ones(1,M)*hydro_genpawn_KS_Tmdummy,'r','LineWidth',2)
title('Model performance (median stat)')
subplot(2,2,3); boxplot1(hydro_genpawn_mean_Tm,X_Labels,'pawn')
hold on; plot((1:M),ones(1,M)*hydro_genpawn_KS_Tmdummy,'r','LineWidth',2)
title('Model performance (mean stat)')
subplot(2,2,4); boxplot1(hydro_genpawn_min_Tm,X_Labels,'pawn') 
hold on; plot((1:M),ones(1,M)*hydro_genpawn_KS_Tmdummy,'r','LineWidth',2)
title('Model performance (min stat)')

% Use bootstrapping to assess robustness of PAWN indices:
% Confidence intervals
alfa  = 0.05 ; % confidence level for intervals around sensitivity indices
nboot = 100;
[hydro_genpawn_KS_median,hydro_genpawn_KS_mean,hydro_genpawn_KS_max,hydro_genpawn_KS_dummy] = pawn_indices_givendata(X,hydro_multi,n,nboot) ;
hydro_genpawn_median_lb = sort(hydro_genpawn_KS_median) ; hydro_genpawn_median_lb = hydro_genpawn_median_lb(max(1,round(nboot*alfa/2)),:);
hydro_genpawn_median_ub = sort(hydro_genpawn_KS_median) ; hydro_genpawn_median_ub = hydro_genpawn_median_ub(round(nboot*(1-alfa/2)),:);
hydro_genpawn_median_Tm = mean(hydro_genpawn_KS_median);
hydro_genpawn_max_lb = sort(hydro_genpawn_KS_max) ; hydro_genpawn_max_lb = hydro_genpawn_max_lb(max(1,round(nboot*alfa/2)),:);
hydro_genpawn_max_ub = sort(hydro_genpawn_KS_max) ; hydro_genpawn_max_ub = hydro_genpawn_max_ub(round(nboot*(1-alfa/2)),:);
hydro_genpawn_max_Tm = mean(hydro_genpawn_KS_max);

% Plot:
figure;
subplot(1,2,1); boxplot1(hydro_genpawn_max_Tm,X_Labels,'pawn',hydro_genpawn_max_lb,hydro_genpawn_max_ub)
hold on; plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (max stat)')
subplot(1,2,2); boxplot1(hydro_genpawn_median_Tm,X_Labels,'pawn',hydro_genpawn_median_lb,hydro_genpawn_median_ub)
hold on; plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (median stat)')

disp('Done'); disp(' ')
%% 46. APPLY PAWN GENERIC SAMPLING FOR THE HYDROLOGICAL MODULE AND FOR MIN AND MAX FLOWS
disp('APPLY PAWN GENERIC SAMPLING FOR THE HYDROLOGICAL MODULE AND FOR MIN AND MAX FLOWS')

% Estimate unconditional and conditional CDFs:
% hydro_pawn_Yu = Qsim;
hydro_genpawn_maxQ = nanmax(Qsim,[],2);
hydro_genpawn_minQ = nanmin(Qsim,[],2);
hydro_genpawn_meanQ = nanmean(Qsim,2);
...
nboot = 100;
% Peak flow
[~,~,hydro_genpawn_maxQ_KS_max,hydro_genpawn_maxQ_KS_dummy] = pawn_indices_givendata(X,hydro_genpawn_maxQ,n,nboot) ;
hydro_genpawn_maxQ_max_lb = sort(hydro_genpawn_maxQ_KS_max) ; hydro_genpawn_maxQ_max_lb = hydro_genpawn_maxQ_max_lb(max(1,round(nboot*alfa/2)),:);
hydro_genpawn_maxQ_max_ub = sort(hydro_genpawn_maxQ_KS_max) ; hydro_genpawn_maxQ_max_ub = hydro_genpawn_maxQ_max_ub(round(nboot*(1-alfa/2)),:);
hydro_genpawn_maxQ_max_Tm = mean(hydro_genpawn_maxQ_KS_max);
% Mean flow
[~,~,hydro_genpawn_meanQ_KS_max,hydro_genpawn_meanQ_KS_dummy] = pawn_indices_givendata(X,hydro_genpawn_meanQ,n,nboot) ;
hydro_genpawn_meanQ_max_lb = sort(hydro_genpawn_meanQ_KS_max) ; hydro_genpawn_meanQ_max_lb = hydro_genpawn_meanQ_max_lb(max(1,round(nboot*alfa/2)),:);
hydro_genpawn_meanQ_max_ub = sort(hydro_genpawn_meanQ_KS_max) ; hydro_genpawn_meanQ_max_ub = hydro_genpawn_meanQ_max_ub(round(nboot*(1-alfa/2)),:);
hydro_genpawn_meanQ_max_Tm = mean(hydro_genpawn_meanQ_KS_max);
% Min flow
[~,~,hydro_genpawn_minQ_KS_max,hydro_genpawn_minQ_KS_dummy] = pawn_indices_givendata(X,hydro_genpawn_minQ,n,nboot) ;
hydro_genpawn_minQ_max_lb = sort(hydro_genpawn_minQ_KS_max) ; hydro_genpawn_minQ_max_lb = hydro_genpawn_minQ_max_lb(max(1,round(nboot*alfa/2)),:);
hydro_genpawn_minQ_max_ub = sort(hydro_genpawn_minQ_KS_max) ; hydro_genpawn_minQ_max_ub = hydro_genpawn_minQ_max_ub(round(nboot*(1-alfa/2)),:);
hydro_genpawn_minQ_max_Tm = mean(hydro_genpawn_minQ_KS_max);

% Plot:
figure; boxplot1(hydro_genpawn_maxQ_max_Tm,X_Labels,'pawn',hydro_genpawn_maxQ_max_lb,hydro_genpawn_maxQ_max_ub)
hold on; plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_maxQ_KS_dummy),'r','LineWidth',2)
title('Peak flows')
figure; boxplot1(hydro_genpawn_meanQ_max_Tm,X_Labels,'pawn',hydro_genpawn_meanQ_max_lb,hydro_genpawn_meanQ_max_ub)
hold on; plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_meanQ_KS_dummy),'r','LineWidth',2)
title('Mean flows')
figure; boxplot1(hydro_genpawn_minQ_max_Tm,X_Labels,'pawn',hydro_genpawn_minQ_max_lb,hydro_genpawn_minQ_max_ub)
hold on; plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_minQ_KS_dummy),'r','LineWidth',2)
title('Min flows')

disp('Done'); disp(' ')
%% 47. APPLY PAWN GENERIC SAMPLING FOR THE EROSION MODULE WITH BEHAVIOURAL HYDROLOGICAL SETS
disp('APPLY PAWN GENERIC SAMPLING FOR THE EROSION MODULE WITH BEHAVIOURAL HYDROLOGICAL SETS')
...
alfa  = 0.05 ; % confidence level for intervals around sensitivity indices
nboot = 100;
[erosion_genpawn_KS_median,erosion_genpawn_KS_mean,erosion_genpawn_KS_max,erosion_genpawn_KS_dummy] = pawn_indices_givendata(Y,erosion_multi,n,nboot) ;
erosion_genpawn_median_lb = sort(erosion_genpawn_KS_median) ; erosion_genpawn_median_lb = erosion_genpawn_median_lb(max(1,round(nboot*alfa/2)),:);
erosion_genpawn_median_ub = sort(erosion_genpawn_KS_median) ; erosion_genpawn_median_ub = erosion_genpawn_median_ub(round(nboot*(1-alfa/2)),:);
erosion_genpawn_median_Tm = mean(erosion_genpawn_KS_median);
erosion_genpawn_max_lb = sort(erosion_genpawn_KS_max) ; erosion_genpawn_max_lb = erosion_genpawn_max_lb(max(1,round(nboot*alfa/2)),:);
erosion_genpawn_max_ub = sort(erosion_genpawn_KS_max) ; erosion_genpawn_max_ub = erosion_genpawn_max_ub(round(nboot*(1-alfa/2)),:);
erosion_genpawn_max_Tm = mean(erosion_genpawn_KS_max);

% Plot:
figure;
subplot(1,2,1); boxplot1(erosion_genpawn_max_Tm,Y_Labels,'pawn',erosion_genpawn_max_lb,erosion_genpawn_max_ub)
hold on; plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (max stat)')
subplot(1,2,2); boxplot1(erosion_genpawn_median_Tm,Y_Labels,'pawn',erosion_genpawn_median_lb,erosion_genpawn_median_ub)
hold on; plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (median stat)')

disp('Done'); disp(' ')
%% 48. APPLY PAWN GENERIC SAMPLING FOR THE EROSION MODULE WITH BEHAVIOURAL HYDROLOGICAL SETS AND FOR PEAK CONCENTRATIONS
disp('APPLY PAWN GENERIC SAMPLING FOR THE EROSION MODULE WITH BEHAVIOURAL HYDROLOGICAL SETS AND FOR PEAK CONCENTRATIONS')

% Estimate unconditional and conditional CDFs:
% erosion_pawn_C = Csim;
erosion_genpawn_maxC = nanmax(Csim,[],2);
erosion_genpawn_meanC = nanmean(Csim,2);
...
nboot = 100;
% Peak concentration
[~,~,erosion_genpawn_maxC_KS_max,erosion_genpawn_maxC_KS_dummy] = pawn_indices_givendata(Y,erosion_genpawn_maxC,n,nboot) ;
erosion_genpawn_maxC_max_lb = sort(erosion_genpawn_maxC_KS_max) ; erosion_genpawn_maxC_max_lb = erosion_genpawn_maxC_max_lb(max(1,round(nboot*alfa/2)),:);
erosion_genpawn_maxC_max_ub = sort(erosion_genpawn_maxC_KS_max) ; erosion_genpawn_maxC_max_ub = erosion_genpawn_maxC_max_ub(round(nboot*(1-alfa/2)),:);
erosion_genpawn_maxC_max_Tm = mean(erosion_genpawn_maxC_KS_max);
% Mean concentration
[~,~,erosion_genpawn_meanC_KS_max,erosion_genpawn_meanC_KS_dummy] = pawn_indices_givendata(Y,erosion_genpawn_meanC,n,nboot) ;
erosion_genpawn_meanC_max_lb = sort(erosion_genpawn_meanC_KS_max) ; erosion_genpawn_meanC_max_lb = erosion_genpawn_meanC_max_lb(max(1,round(nboot*alfa/2)),:);
erosion_genpawn_meanC_max_ub = sort(erosion_genpawn_meanC_KS_max) ; erosion_genpawn_meanC_max_ub = erosion_genpawn_meanC_max_ub(round(nboot*(1-alfa/2)),:);
erosion_genpawn_meanC_max_Tm = mean(erosion_genpawn_meanC_KS_max);

% Plot:
figure; boxplot1(erosion_genpawn_maxC_max_Tm,Y_Labels,'pawn',erosion_genpawn_maxC_max_lb,erosion_genpawn_maxC_max_ub)
hold on; plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_maxC_KS_dummy),'r','LineWidth',2)
title('Peak concentrations')
figure; boxplot1(erosion_genpawn_meanC_max_Tm,Y_Labels,'pawn',erosion_genpawn_meanC_max_lb,erosion_genpawn_meanC_max_ub)
hold on; plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_meanC_KS_dummy),'r','LineWidth',2)
title('Mean concentrations')

disp('Done'); disp(' ')
%% 49. APPLY PAWN GENERIC SAMPLING FOR THE RANDOM EROSION MODULE
disp('APPLY PAWN GENERIC SAMPLING FOR THE RANDOM EROSION MODULE')
...
alfa  = 0.05 ; % confidence level for intervals around sensitivity indices
nboot = 100;
[rand_erosion_genpawn_KS_median,rand_erosion_genpawn_KS_mean,rand_erosion_genpawn_KS_max,rand_erosion_genpawn_KS_dummy] = pawn_indices_givendata(XY2,rand_erosion_multi,n,nboot) ;
rand_erosion_genpawn_median_lb = sort(rand_erosion_genpawn_KS_median) ; rand_erosion_genpawn_median_lb = rand_erosion_genpawn_median_lb(max(1,round(nboot*alfa/2)),:);
rand_erosion_genpawn_median_ub = sort(rand_erosion_genpawn_KS_median) ; rand_erosion_genpawn_median_ub = rand_erosion_genpawn_median_ub(round(nboot*(1-alfa/2)),:);
rand_erosion_genpawn_median_Tm = mean(rand_erosion_genpawn_KS_median);
rand_erosion_genpawn_max_lb = sort(rand_erosion_genpawn_KS_max) ; rand_erosion_genpawn_max_lb = rand_erosion_genpawn_max_lb(max(1,round(nboot*alfa/2)),:);
rand_erosion_genpawn_max_ub = sort(rand_erosion_genpawn_KS_max) ; rand_erosion_genpawn_max_ub = rand_erosion_genpawn_max_ub(round(nboot*(1-alfa/2)),:);
rand_erosion_genpawn_max_Tm = mean(rand_erosion_genpawn_KS_max);

% Plot:
figure;
subplot(1,2,1); boxplot1(rand_erosion_genpawn_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_max_lb,rand_erosion_genpawn_max_ub)
hold on; plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (max stat)')
subplot(1,2,2); boxplot1(rand_erosion_genpawn_median_Tm,XY_Labels,'pawn',rand_erosion_genpawn_median_lb,rand_erosion_genpawn_median_ub)
hold on; plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_KS_dummy),'r','LineWidth',2)
title('Model performance (median stat)')

disp('Done'); disp(' ')
%% 50. APPLY PAWN GENERIC SAMPLING FOR THE RANDOM EROSION MODULE AND FOR PEAK CONCENTRATIONS
disp('APPLY PAWN GENERIC SAMPLING FOR THE RANDOM EROSION MODULE AND FOR PEAK CONCENTRATIONS')

% Estimate unconditional and conditional CDFs:
% rand_erosion_genpawn_C = C2sim;
rand_erosion_genpawn_maxC = nanmax(C2sim,[],2);
rand_erosion_genpawn_meanC = nanmean(C2sim,2);
...
nboot = 100;
% Peak concentration
[~,~,rand_erosion_genpawn_maxC_KS_max,rand_erosion_genpawn_maxC_KS_dummy] = pawn_indices_givendata(XY2,rand_erosion_genpawn_maxC,n,nboot) ;
rand_erosion_genpawn_maxC_max_lb = sort(rand_erosion_genpawn_maxC_KS_max) ; rand_erosion_genpawn_maxC_max_lb = rand_erosion_genpawn_maxC_max_lb(max(1,round(nboot*alfa/2)),:);
rand_erosion_genpawn_maxC_max_ub = sort(rand_erosion_genpawn_maxC_KS_max) ; rand_erosion_genpawn_maxC_max_ub = rand_erosion_genpawn_maxC_max_ub(round(nboot*(1-alfa/2)),:);
rand_erosion_genpawn_maxC_max_Tm = mean(rand_erosion_genpawn_maxC_KS_max);
% Mean concentration
[~,~,rand_erosion_genpawn_meanC_KS_max,rand_erosion_genpawn_meanC_KS_dummy] = pawn_indices_givendata(XY2,rand_erosion_genpawn_meanC,n,nboot) ;
rand_erosion_genpawn_meanC_max_lb = sort(rand_erosion_genpawn_meanC_KS_max) ; rand_erosion_genpawn_meanC_max_lb = rand_erosion_genpawn_meanC_max_lb(max(1,round(nboot*alfa/2)),:);
rand_erosion_genpawn_meanC_max_ub = sort(rand_erosion_genpawn_meanC_KS_max) ; rand_erosion_genpawn_meanC_max_ub = rand_erosion_genpawn_meanC_max_ub(round(nboot*(1-alfa/2)),:);
rand_erosion_genpawn_meanC_max_Tm = mean(rand_erosion_genpawn_meanC_KS_max);

% Plot:
figure; boxplot1(rand_erosion_genpawn_maxC_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_maxC_max_lb,rand_erosion_genpawn_maxC_max_ub)
hold on; plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_maxC_KS_dummy),'r','LineWidth',2)
title('Peak concentrations')
figure; boxplot1(rand_erosion_genpawn_meanC_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_meanC_max_lb,rand_erosion_genpawn_meanC_max_ub)
hold on; plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_meanC_KS_dummy),'r','LineWidth',2)
title('Mean concentrations')

disp('Done'); disp(' ')
%% 51. PLOT SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS
disp('PLOT SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS')
...
% figure
subplot(2,2,2)
boxplot1([hydro_genpawn_max_Tm,erosion_genpawn_max_Tm],XY_Labels,'pawn',[hydro_genpawn_max_lb,erosion_genpawn_max_lb],[hydro_genpawn_max_ub,erosion_genpawn_max_ub],10)
hold on
plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_KS_dummy),'r','LineWidth',2)
plot((M:M+E+1),ones(1,E+2)*mean(erosion_genpawn_KS_dummy),'r','LineWidth',2)
plot([M M]+0.5,[-0.5 1.5],'--k')
grid on
title('(c) PAWN SA of the hydrological and erosion modules')
set(gca,'XTickLabelRotation',90)
...
subplot(2,2,4)
boxplot1(rand_erosion_genpawn_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_max_lb,rand_erosion_genpawn_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_KS_dummy),'r','LineWidth',2)
grid on
title('(d) PAWN SA of the erosion module only')
set(gca,'XTickLabelRotation',90)

disp('Done'); disp(' ')
%% 52. PLOT SENSITIVITY INDICES (PAWN GEN SAMPL) OF OUTPUT ANALYSIS
disp('PLOT SENSITIVITY INDICES (PAWN GEN SAMPL) OF OUTPUT ANALYSIS')
...
subplot(3,(M+ME+2)*np,(1:M*np))
boxplot1(hydro_genpawn_maxQ_max_Tm,X_Labels,'pawn',hydro_genpawn_maxQ_max_lb,hydro_genpawn_maxQ_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(hydro_genpawn_maxQ_KS_dummy),'r','LineWidth',2)
grid on
title('(a) Sensitivity in peak flow simulation')
set(gca,'XTickLabelRotation',90)

subplot(3,(M+ME+2)*np,(1:M*np)+(M+ME+2)*np)
boxplot1(hydro_genpawn_meanQ_max_Tm,X_Labels,'pawn',hydro_genpawn_meanQ_max_lb,hydro_genpawn_meanQ_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(hydro_genpawn_meanQ_KS_dummy),'r','LineWidth',2)
grid on
title('(b) Sensitivity in mean flow simulation')
set(gca,'XTickLabelRotation',90)
...
subplot(3,(M+ME+2)*np,(1:M*np)+(M+ME+2)*np*2)
title('(c) Sensitivity in minimum flow simulation')
boxplot1(hydro_genpawn_minQ_max_Tm,X_Labels,'pawn',hydro_genpawn_minQ_max_lb,hydro_genpawn_minQ_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(hydro_genpawn_minQ_KS_dummy),'r','LineWidth',2)
grid on
set(gca,'XTickLabelRotation',90)

subplot(3,(M+ME+2)*np,((M+ME+2)*3*np-E*np-1:(M+ME+2)*3*np)-(M+ME+2)*np*2)
boxplot1(erosion_genpawn_maxC_max_Tm,Y_Labels,'pawn',erosion_genpawn_maxC_max_lb,erosion_genpawn_maxC_max_ub,10)
hold on
plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_maxC_KS_dummy),'r','LineWidth',2)
grid on
t = title('(d) Sensitivity in peak sediment concentration (behavioural sets)');
set(gca,'XTickLabelRotation',90)
tpox = get(t,'Position'); tpox(1) = -2.5;
...
subplot(3,(M+ME+2)*np,((M+ME+2)*3*np-ME*np+1:(M+ME+2)*3*np)-(M+ME+2)*np)
boxplot1(rand_erosion_genpawn_maxC_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_maxC_max_lb,rand_erosion_genpawn_maxC_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_maxC_KS_dummy),'r','LineWidth',2)
grid on
title('(e) Sensitivity in peak sediment concentration (random sets)')
set(gca,'XTickLabelRotation',90)%,'YTick',[],'YTickLabels',[],'ylabel',[])

subplot(3,(M+ME+2)*np,((M+ME+2)*3*np-ME*np+1:(M+ME+2)*3*np))
boxplot1(rand_erosion_genpawn_meanC_max_Tm,XY_Labels,'pawn',rand_erosion_genpawn_meanC_max_lb,rand_erosion_genpawn_meanC_max_ub,10)
hold on
plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_meanC_KS_dummy),'r','LineWidth',2)
grid on
title('(f) Sensitivity in mean sediment concentration')
set(gca,'XTickLabelRotation',90)%,'YTick',[],'YTickLabels',[],'ylabel',[])
...
disp('Done'); disp(' ')
%% 53. BAR PLOT OF SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS
disp('BAR PLOT OF SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS')
...
for i = 1:M
    b = bar(i,hydro_mvd(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_mvd(i))
end
for i = 1:E
    b = bar(M+i,erosion_mvd(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*erosion_mvd(i))
end
ylabel('mvd','FontSize',fs)
axis([0,M+E+1,0,1])
plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(a) RSA of the hydrological and erosion modules','FontSize',fs)
title('(a) RSA of the hydrological and erosion modules')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:M+E),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_mvd_lb(i) hydro_mvd_ub(i)],'k','LineWidth',1.5);     
end
for i = 1:E
    plot(M+[i i],[erosion_mvd_lb(i) erosion_mvd_ub(i)],'k','LineWidth',1.5);     
end
set(fig_4, 'Position', [0 0 1200 1100])

subplot(4,1,2)
for i = 1:ME
    b = bar(i,rand_erosion_mvd(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_mvd(i))
end
ylabel('mvd','FontSize',fs)
axis([0,ME+1,0,1])
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(b) RSA of the erosion module only','FontSize',fs)
title('(b) RSA of the erosion module only')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:ME
    plot([i i],[rand_erosion_mvd_lb(i) rand_erosion_mvd_ub(i)],'k','LineWidth',1.5);     
end

% Plot bar chart PAWN
subplot(4,1,3)
for i = 1:M
    b = bar(i,hydro_genpawn_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_genpawn_max_Tm(i))
end
for i = 1:E
    b = bar(M+i,erosion_genpawn_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*erosion_genpawn_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,M+E+1,0,1])
plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_KS_dummy),'k','LineWidth',1.5)
plot((M:M+E+1),ones(1,E+2)*mean(erosion_genpawn_KS_dummy),'k','LineWidth',1.5)
plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(cs) PAWN SA of the hydrological and erosion modules','FontSize',fs)
title('(c) PAWN SA of the hydrological and erosion modules')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:M+E),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_genpawn_max_lb(i) hydro_genpawn_max_ub(i)],'k','LineWidth',1.5);     
end
for i = 1:E
    plot(M+[i i],[erosion_genpawn_max_lb(i) erosion_genpawn_max_ub(i)],'k','LineWidth',1.5);     
end
set(fig_4, 'Position', [0 0 1200 1100])

subplot(4,1,4)
for i = 1:ME
    b = bar(i,rand_erosion_genpawn_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_genpawn_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,ME+1,0,1])
plot((0:ME+1),ones(1,ME+2)*mean(rand_erosion_genpawn_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(d) PAWN SA of the erosion module only','FontSize',fs)
title('(d) PAWN SA of the erosion module only')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:ME
    plot([i i],[rand_erosion_genpawn_max_lb(i) rand_erosion_genpawn_max_ub(i)],'k','LineWidth',1.5);     
end
...
disp('Done'); disp(' ')
%% 54. ALTERNATIVE BAR PLOT OF SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS
disp('ALTERNATIVE BAR PLOT OF SENSITIVITY INDICES (MVD AND PAWN GEN SAMPL) OF BEHAVIOURAL AND RANDOM SETUPS')

% Initialise the Figure
fig_4alt = figure;
set(fig_4alt,'Renderer','painters')
...
plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(a) RSA of the hydrological and erosion modules','FontSize',fs)
title('(a) RSA of the hydrological and erosion modules (NSE, PBIAS, KGE)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:M+E),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
...
set(fig_4alt, 'Position', [0 0 1200 1100])
...
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(b) RSA of the erosion module only','FontSize',fs)
title('(b) RSA of the erosion module only (NSE, PBIAS, KGE)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
...
subplot(6,1,3)
for i = 1:M
    b = bar(i,hydro_multi_mvd(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_multi_mvd(i))
end
for i = 1:E
    b = bar(M+i,erosion_multi_mvd(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*erosion_multi_mvd(i))
end
ylabel('mvd','FontSize',fs)
axis([0,M+E+1,0,1])
plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(a) RSA of the hydrological and erosion modules','FontSize',fs)
title('(c) RSA of the hydrological and erosion modules (MultiObj)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:M+E),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_multi_mvd_lb(i) hydro_multi_mvd_ub(i)],'k','LineWidth',1.5);     
end
for i = 1:E
    plot(M+[i i],[erosion_multi_mvd_lb(i) erosion_multi_mvd_ub(i)],'k','LineWidth',1.5);     
end
set(fig_4alt, 'Position', [0 0 1200 1100])

subplot(6,1,4)
for i = 1:ME
    b = bar(i,rand_erosion_multi_mvd(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_multi_mvd(i))
end
ylabel('mvd','FontSize',fs)
axis([0,ME+1,0,1])
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(b) RSA of the erosion module only','FontSize',fs)
title('(d) RSA of the erosion module only (MultiObj)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:ME
    plot([i i],[rand_erosion_multi_mvd_lb(i) rand_erosion_multi_mvd_ub(i)],'k','LineWidth',1.5);     
end
...
plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(cs) PAWN SA of the hydrological and erosion modules','FontSize',fs)
title('(e) PAWN SA of the hydrological and erosion modules (MultiObj)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:M+E),'XTickLabel',{})
% set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
...
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(d) PAWN SA of the erosion module only','FontSize',fs)
title('(f) PAWN SA of the erosion module only (MultiObj)')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
set(gca,'XTickLabel',XY_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
...
disp('Exporting the Figure to pdf')
% Set the paper size [width height]
set(fig_4alt,'PaperSize',[20 24]);
% set(fig_4alt,'PaperOrientation','landscape')
print(fig_4alt,'Fig4alt_export','-dpdf','-fillpage')

clear b fig_4

disp('Done'); disp(' ')
%% 55. BAR PLOT OF SENSITIVITY INDICES (PAWN GEN SAMPL) FOR MODEL OUTPUTS
disp('BAR PLOT OF SENSITIVITY INDICES (PAWN GEN SAMPL) FOR MODEL OUTPUTS')

% Initialise the Figure
fig_5 = figure;
set(fig_5,'Renderer','painters')

fs = 10 ;
np = 0;

% Peak Flow
subplot(5,ME+np,0*(ME+np)+(1:M))
for i = 1:M
    b = bar(i,hydro_genpawn_maxQ_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_genpawn_maxQ_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,M+.6,0,1])
plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_maxQ_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(a) Sensitivity for peak flow simulation','FontSize',fs)
title('(a) Sensitivity for peak flow simulation')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',X_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_genpawn_maxQ_max_lb(i) hydro_genpawn_maxQ_max_ub(i)],'k','LineWidth',1.5);     
end

% Mean Flow
subplot(5,ME+np,1*(ME+np)+(1:M))
for i = 1:M
    b = bar(i,hydro_genpawn_meanQ_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_genpawn_meanQ_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,M+.6,0,1])
plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_meanQ_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(b) Sensitivity for mean flow simulation','FontSize',fs)
title('(b) Sensitivity for mean flow simulation')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',X_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_genpawn_meanQ_max_lb(i) hydro_genpawn_meanQ_max_ub(i)],'k','LineWidth',1.5);     
end

% Min Flow
subplot(5,ME+np,2*(ME+np)+(1:M))
for i = 1:M
    b = bar(i,hydro_genpawn_minQ_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[1 0.5 0.25]*hydro_genpawn_minQ_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,M+.6,0,1])
plot((0:M+1),ones(1,M+2)*mean(hydro_genpawn_minQ_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(c) Sensitivity for minimum flow simulation','FontSize',fs)
title('(c) Sensitivity for minimum flow simulation')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{})
% set(gca,'XTickLabel',X_Labels,'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[hydro_genpawn_minQ_max_lb(i) hydro_genpawn_minQ_max_ub(i)],'k','LineWidth',1.5);     
end

% % Mean Concentration behavioural
% subplot(5,ME+np,1*(ME+np)+(M+1+np:ME+np))
% for i = 1:E
%     b = bar(i,erosion_genpawn_meanC_max_Tm(i));
%     hold on
%     set(b,'FaceColor',1-[1 1 1]*erosion_genpawn_meanC_max_Tm(i))
% end
% % ylabel('pawn','FontSize',fs)
% axis([0,E+1,0,1])
% plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_meanC_KS_dummy),'k','LineWidth',1.5)
% % plot([M M]+0.5,[-0.5 1.5],'--k')
% % text(1,0.8,'(d) mean sediment concentration','FontSize',fs)
% title('(d) mean sediment concentration')
% % add axis ticks and label
% set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{},'YTickLabel',{})
% % set(gca,'XTickLabel',Y_Labels,'XTickLabelRotation',90)
% % connect upper and lower bound with a line
% for i = 1:E
%     plot([i i],[erosion_genpawn_meanC_max_lb(i) erosion_genpawn_meanC_max_ub(i)],'k','LineWidth',1.5);     
% end
% 
% % Peak Concentration behavioural
% subplot(5,ME+np,2*(ME+np)+(M+1+np:ME+np))
% for i = 1:E
%     b = bar(i,erosion_genpawn_maxC_max_Tm(i));
%     hold on
%     set(b,'FaceColor',1-[1 1 1]*erosion_genpawn_maxC_max_Tm(i))
% end
% % ylabel('pawn','FontSize',fs)
% axis([0,E+1,0,1])
% plot((0:E+1),ones(1,E+2)*mean(erosion_genpawn_maxC_KS_dummy),'k','LineWidth',1.5)
% % plot([M M]+0.5,[-0.5 1.5],'--k')
% % text(1,0.8,'(e) peak sediment concentration','FontSize',fs)
% title('(e) peak sediment concentration')
% % add axis ticks and label
% set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME),'XTickLabel',{},'YTickLabel',{})
% % set(gca,'XTickLabel',Y_Labels,'XTickLabelRotation',90)
% % connect upper and lower bound with a line
% for i = 1:E
%     plot([i i],[erosion_genpawn_maxC_max_lb(i) erosion_genpawn_maxC_max_ub(i)],'k','LineWidth',1.5);     
% end

% Peak Concentration random
subplot(5,ME+np,3*(ME+np)+(1:ME+np))
for i = 1:M
    b = bar(i,rand_erosion_genpawn_maxC_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_genpawn_maxC_max_Tm(i))
end
for i = M+1:ME
    b = bar(i+np,rand_erosion_genpawn_maxC_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_genpawn_maxC_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,ME+1+np,0,1])
plot((0:ME+1+np),ones(1,ME+2+np)*mean(rand_erosion_genpawn_maxC_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(d) Sensitivity for peak sediment concentration simulation','FontSize',fs)
title('(d) Sensitivity for peak sediment concentration simulation')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME+np),'XTickLabel',{})
% set(gca,'XTickLabel',[X_Labels,repmat({''},1,np),Y_Labels],'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[rand_erosion_genpawn_maxC_max_lb(i) rand_erosion_genpawn_maxC_max_ub(i)],'k','LineWidth',1.5);     
end
for i = M+1:ME
    plot([i i]+np,[rand_erosion_genpawn_maxC_max_lb(i) rand_erosion_genpawn_maxC_max_ub(i)],'k','LineWidth',1.5);     
end

% Mean Concentration random
subplot(5,ME+np,4*(ME+np)+(1:ME+np))
for i = 1:M
    b = bar(i,rand_erosion_genpawn_meanC_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_genpawn_meanC_max_Tm(i))
end
for i = M+1:ME
    b = bar(i+np,rand_erosion_genpawn_meanC_max_Tm(i));
    hold on
    set(b,'FaceColor',1-[0.5 0.5 1]*rand_erosion_genpawn_meanC_max_Tm(i))
end
ylabel('pawn','FontSize',fs)
axis([0,ME+1+np,0,1])
plot((0:ME+1+np),ones(1,ME+2+np)*mean(rand_erosion_genpawn_meanC_KS_dummy),'k','LineWidth',1.5)
% plot([M M]+0.5,[-0.5 1.5],'--k')
% text(1,0.8,'(e) Sensitivity for mean sediment concentration simulation','FontSize',fs)
title('(e) Sensitivity for mean sediment concentration simulation')
% add axis ticks and label
set(gca,'FontSize',fs,'YTick',(0:0.2:1),'XTick',(1:ME+np),'XTickLabel',{})
set(gca,'XTickLabel',[X_Labels,repmat({''},1,np),Y_Labels],'XTickLabelRotation',90)
% connect upper and lower bound with a line
for i = 1:M
    plot([i i],[rand_erosion_genpawn_meanC_max_lb(i) rand_erosion_genpawn_meanC_max_ub(i)],'k','LineWidth',1.5);     
end
for i = M+1:ME
    plot([i i]+np,[rand_erosion_genpawn_meanC_max_lb(i) rand_erosion_genpawn_meanC_max_ub(i)],'k','LineWidth',1.5);     
end

% set(fig_5, 'Position', [0 0 1200 1100])

% Export Figure
disp('Exporting the Figure to pdf')
% Set the paper size [width height]
set(fig_5,'PaperSize',[20 20]);
% set(fig_5,'PaperOrientation','landscape')
print(fig_5,'Fig5_export','-dpdf','-fillpage')

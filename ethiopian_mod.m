function [Q_sim,STATES,FLUXES] = ethiopian_mod(param,rain,evap)
%
...
A1    = param(1)/sum(param(1:3)); % Fraction of saturated area [-]
A2    = param(2)/sum(param(1:3)); % Fraction of degraded area [-]
A3    = param(3)/sum(param(1:3)); % Fraction of infiltration area [-]
...
inter = zeros(N+1,1); % Baseflow reservoir moisture [mm]
perc = zeros(N,1); % Percolation in area 3 [mm/Dt]
BF = zeros(N,1); % Baseflow [mm/Dt]
IF = zeros(N,1); % Interflow [mm/Dt]
...
inter(1) = 0; % Initial Interflow Reservoir Level [mm]
...
Ea = A1*Ea1 + A2*Ea2 + A3*Ea3; % Total actual evapotranspiration
RF1 = A1*runoff1; % Runoff produced by area 1
RF2 = A2*runoff2; % Runoff produced by area 2
RF = RF1 + RF2; % Total runoff is produced by areas 1 and 2
rech = A3*rech; % Recharge is only produced beneath area 3
...
for t=1:N
    ...
    % Interflow dynamics
    inter(t+1) = perc(t)+inter(t);
    IF(t) = 2*inter(t+1)*(1/tau - 0.5/tau^2);
    inter(t+1) = inter(t+1) - IF(t);
end
...
STATES=[soil1,soil2,soil3,base,inter];
